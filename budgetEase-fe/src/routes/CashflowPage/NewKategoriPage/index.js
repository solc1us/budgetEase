...
const SamplePage = () => {
  ...
  const onFinishDelete = (values) => {
    ...
    axios({
      method: "delete",
      url: "http://localhost:8080/kategori/deletebyid",
      data: {
        id: values.id
      },
    })
      .then(function (response) {
        ...
        history.push("/cashflow-newkategori");
      })
      .catch(function (error) {
        console.log(error);
      })
  ...
  return (
    <div>
      ...
      <div className="gx-main-user-container gx-rounded-lg">
        ...
        <div className="gx-flex-column gx-px-3">
          <Form onFinish={onFinishKategori} onFinishFailed={onFinishFailed}>
            ...
            <Form.Item name="existingkategori" label="Kategori yang ada">
              <Select>
                {kategori.map((item) => (
                  <Select.Option
                    disabled
      ...
      <div className="gx-main-user-container gx-rounded-lg">
        <div className="gx-main-user-table-filter">
          <h1 className="gx-mb-4">Delete Kategori!</h1>
        </div>
        <div className="gx-flex-column gx-px-3">
          <Form onFinish={onFinishDelete} onFinishFailed={onFinishFailed}>
            <Form.Item name="id" label="Kategori">
              <Select>
                {kategori.map((item) => (
                  <Select.Option
                    key={item.id}
